  ...
  if (clang::NamedDecl *nd = llvm::dyn_cast<clang::NamedDecl>(d)) {
    ...
    if (clang::FunctionDecl *fd = llvm::dyn_cast<clang::FunctionDecl>(d)) {
      if (fd->getTemplateInstantiationPattern() != NULL) {
        ...
        fd_loc = spellingloc;
        RefType refType =
            fd->isThisDeclarationADefinition() ? RT_Definition : RT_Declaration;
        ...
        if (td != nullptr) {
          ...
          if (rd) {
            RecordSymbolDep1(SDT_FieldType_Typedef, rd, td, td->getBeginLoc());
                     ...
                     llvm::dyn_cast<clang::TypedefNameDecl>(d)) {
  ...
  if (fd_ctx && vd) {
    if (clang::VarDecl *var = llvm::dyn_cast<clang::VarDecl>(vd)) {
      if (var->hasGlobalStorage() && var->getStorageClass() != SC_Static) {
        ...
        RecordSymbolDep1(SDT_GlobalVar_Function, fd_ctx, var, e->getBeginLoc());
      }
    }
  }
  return true;
...
//目前支持提取依赖符号类型为a=b+c+d+e+f
//由于a+=b这种依赖关系目前不是binaryOperate所以并没有记录，记录起来不难。

void DARecursiveASTVisitor::recordBinaryOperatorDep(clang::ValueDecl *Vd,clang::Stmt *St,clang::SourceLocation Oloc){
  if (clang::ImplicitCastExpr *Implicitcastexpr = llvm::dyn_cast<clang::ImplicitCastExpr>(St)) {
    clang::Expr *Child = Implicitcastexpr->getSubExpr();
    if(clang::DeclRefExpr *Declrefexpr = llvm::dyn_cast<clang::DeclRefExpr>(Child)){
      clang::ValueDecl *VVd = Declrefexpr->getDecl();
      if (clang::VarDecl *Globvar = llvm::dyn_cast<clang::VarDecl>(VVd)){
        if(Globvar->hasGlobalStorage() && Globvar->getStorageClass() != SC_Static){
          RecordSymbolDep1(SDT_GlobalVar_GlobalVar,Vd,Globvar,Oloc);
        }
      }
    }
  }else{
    if (clang::BinaryOperator *BinaryOp = llvm::dyn_cast<clang::BinaryOperator>(St)){
      ...
      recordBinaryOperatorDep(Vd,Lhs,Oloc);
      recordBinaryOperatorDep(Vd,Rhs,Oloc);
    }
  }
}
...
void DARecursiveASTVisitor::RecordDeclRefExpr(clang::NamedDecl *d,
...
bool DARecursiveASTVisitor::VisitStmt(clang::Stmt *st) {
    ...
    if (binaryOp->isAssignmentOp()) {
      ...
      if (clang::DeclRefExpr *left = llvm::dyn_cast<clang::DeclRefExpr>(lhs)) {
        ...
        if (vd) {
          if (clang::VarDecl *var = llvm::dyn_cast<clang::VarDecl>(vd)) {
            if (var->hasGlobalStorage() && var->getStorageClass() != SC_Static) {
              recordBinaryOperatorDep(var,rhs,Oloc);
            }
          }
        }
      }
  ...
  if (clang::CallExpr *call = llvm::dyn_cast<clang::CallExpr>(st)) {
    ...
    if (!func_decl->isStatic() || !fd_ctx->isStatic())
      RecordSymbolDep1(SDT_Function_Function, fd_ctx, func_decl,
                       call->getBeginLoc());
  }
  return true;
}
...
void DARecursiveASTVisitor::RecordDeclRef(clang::NamedDecl *d,
...
void DARecursiveASTVisitor::RecordSymbolDep1(
    SDependencyType Sdt, clang::NamedDecl *Sd, clang::NamedDecl *Ed,
    clang::SourceLocation OcurrenceLoc) {
  assert(Sd != NULL);
  assert(Ed != NULL);
  if (isNamedDeclUnnamed(Sd) || isNamedDeclUnnamed(Ed))
    return;
  ...
  if (SSpellingLoc.isValid() && ESpellingLoc.isValid() &&
      OSpellingLoc.isValid()) {
  ...
  FSt->addSymbolDependency(Sdt, Ssid, Esid, Sfid, Efid, Slocation, Elocation,
                           Ofid, Olocation);
}
